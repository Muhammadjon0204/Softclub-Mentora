...
public sealed record NotificationMessage(
    ...
    // NTF-017: a token-bearing link is never persisted in the outbox row, so it cannot live in
    // Payload. This is minted by the dispatcher immediately before send, for UserInvitation only,
    // and reaches the sender/template only in memory for this one delivery attempt.
