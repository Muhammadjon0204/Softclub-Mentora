...
public sealed class OutboxDispatcher(
    ...
    // Only for minting a fresh set-password link at send time, for UserInvitation (see DeliverAsync)
    // — NTF-017 forbids putting the token in the outbox row, so it cannot be resolved any earlier.
    AuthService authService)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var actionUrl = await BuildActionUrlAsync(row, cancellationToken);
    ...
    /// <summary>
    /// Mints the one-time link a <c>UserInvitation</c> delivery needs, at the moment of sending.
    /// </summary>
    /// <remarks>
    /// <para>
    /// <c>NTF-017</c> forbids a token in the outbox payload, and the token is stored hashed
    /// (<c>AUTH-019</c>) — so there is no plaintext to recover from the row at all, at any point after
    /// <c>IssueSetPasswordLinkAsync</c> first returned it. The only correct place left to obtain one is
    /// here, freshly, right before the email actually leaves — a retry of this same row mints and sends
    /// a new token each time, superseding whichever one an earlier attempt (or the original
    /// <c>POST /users</c> call) already issued; nothing depends on those staying valid.
    /// </para>
    /// <para>
    /// A user deactivated or deleted between enqueue and send returns null here, and the resulting
    /// email falls back to the bare app link — a stale invitation for someone no longer eligible to
    /// accept it is not an error worth dead-lettering the row over.
    /// </para>
    /// </remarks>
    private async Task<string?> BuildActionUrlAsync(NotificationOutbox row, CancellationToken cancellationToken)
    {
        if (row.EventType != NotificationEventTypes.UserInvitation)
        {
            return null;
        }
