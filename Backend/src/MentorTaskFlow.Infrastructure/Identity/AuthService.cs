    ...
    IClock clock) : IAuthService
{
    ...
    private async Task<AuthUserDto> MapProfileAsync(User user, CancellationToken cancellationToken)
    {
        ...
        {
            category = await (
                    from c in dbContext.Categories.IgnoreQueryFilters()
                    join s in dbContext.CategorySettings.IgnoreQueryFilters() on c.Id equals s.CategoryId
                    where c.Id == categoryId
                    select new CategorySummaryDto(c.Id, c.Name, s.TimeZoneId))
                .AsNoTracking()
                .FirstOrDefaultAsync(cancellationToken);
        }
