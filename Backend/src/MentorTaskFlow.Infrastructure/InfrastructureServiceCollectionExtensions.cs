...
public static class InfrastructureServiceCollectionExtensions
{
    ...
    private static void AddStorage(IServiceCollection services, IConfiguration configuration)
    {
        ...
        // A second client, signing against Storage:PublicEndpoint when one is configured, used only for
        // presigned URLs (see StorageOptions.PublicEndpoint). Falls back to the same endpoint as the
        // primary client, so a deployment that never sets PublicEndpoint gets one identical client under
        // two registrations, not two code paths to maintain.
        services.AddKeyedSingleton<IMinioClient>(MinioClientKeys.Presign, (serviceProvider, _) =>
        {
            var options = serviceProvider.GetRequiredService<IOptions<StorageOptions>>().Value;
            var endpoint = StorageEndpoint.Parse(options.PublicEndpoint ?? options.Endpoint);
