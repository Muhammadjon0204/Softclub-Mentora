...
/// <summary>
/// Minimal Category view embedded in <c>AuthUser</c> (<c>AUTH-038</c>), mirroring
/// <see cref="Tenancy.BranchSummaryDto"/>'s role for Branch.
/// </summary>
/// <remarks>
/// <c>timeZoneId</c> comes from <c>CategorySettings</c>, not <c>Branch</c> — the same distinction
/// <see cref="Tenancy.BranchSummaryDto"/>'s own remarks make: a Lead/Mentor's zone is always the
/// category's, which starts out equal to the branch's at creation but is not tied to it afterwards.
/// </remarks>
public sealed record CategorySummaryDto(Guid Id, string Name, string TimeZoneId);
