...
public sealed class OutboxDeliveryTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    private static OutboxDispatcher Dispatcher(
        ...
        FixedClock? clock)
    {
        var effectiveClock = clock ?? new FixedClock(Seeded);
        ...
        return new(
            ...
            effectiveClock,
            BuildAuthService(context, effectiveClock));
    }

    /// <summary>
    /// A real <c>AuthService</c>, hand-wired rather than pulled from <c>MentorTaskFlowApiFactory</c>:
    /// <c>OutboxDispatcher</c> only ever calls its <c>IssueSetPasswordLinkAsync</c> (see
    /// <c>DeliverAsync</c>'s <c>BuildActionUrlAsync</c>), which never touches <c>IAuditWriter</c>, so
    /// the tenancy/audit dependencies below just need to satisfy the constructor, not do anything real.
    /// </summary>
    private static AuthService BuildAuthService(MentorTaskFlowDbContext context, IClock clock)
    {
        var authOptions = Options.Create(new AuthOptions
        {
            JwtSigningKey = "outbox-delivery-tests-hand-wired-signing-key-32-bytes-min",
        });
